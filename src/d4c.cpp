...
//-----------------------------------------------------------------------------
	static void GetSmoothedPowerSpectrum(const double* x, int x_length, int fs,
		...
		for(int i = 0; i < fft_size; ++i) forward_real_fft->waveform[i] = 0.0;
		GetWindowedWaveform(x, x_length, fs, current_f0, current_position, world::kHanning, 4.0, forward_real_fft->waveform);
...
//-----------------------------------------------------------------------------
	static void D4CLoveTrain(const double* x, int fs, int x_length, const torch::Tensor& f0, int f0_length, const double* temporal_positions, double* aperiodicity0) {
		...
		for(int i = 0; i < f0_length; ++i) {
			...
			aperiodicity0[i] = D4CLoveTrainSub(x, fs, x_length, MyMaxDouble(val, lowest_f0), temporal_positions[i], f0_length, fft_size, boundary0, boundary1, boundary2, &forward_real_fft);
		}
...
//-----------------------------------------------------------------------------
	static void D4CGeneralBody(const double* x, int x_length, int fs,
		...
		GetStaticCentroid(x, x_length, fs, current_f0, fft_size, current_position, forward_real_fft, static_centroid);
		GetSmoothedPowerSpectrum(x, x_length, fs, current_f0, fft_size, current_position, forward_real_fft, smoothed_power_spectrum);
	...
	static void InitializeAperiodicity(int f0_length, int fft_size, const torch::Tensor& aperiodicity) {
		for(int i = 0; i < f0_length; ++i)
	...
	static void GetAperiodicity(const double* coarse_frequency_axis, const double* coarse_aperiodicity, int number_of_aperiodicities, const double* frequency_axis, int fft_size, const torch::Tensor& aperiodicity) {
		interp1(coarse_frequency_axis, coarse_aperiodicity,number_of_aperiodicities + 2, frequency_axis, fft_size / 2 + 1, aperiodicity.to(c10::kDouble).data_ptr<double>());
		for(int i = 0; i <= fft_size / 2; ++i)
...
void D4C(const double* x, int x_length, int fs, const double* temporal_positions, const torch::Tensor& f0, int f0_length, int fft_size, const D4COption* option, const torch::Tensor& aperiodicity) {
	randn_reseed();
	...
	int number_of_aperiodicities =
		  static_cast<int>(MyMinDouble(world::kUpperLimit, fs / 2.0 - world::kFrequencyInterval) / world::kFrequencyInterval);
	...
	D4CLoveTrain(x, fs, x_length, f0, f0_length, temporal_positions, aperiodicity0);
	...
	for(int i = 0; i < f0_length; ++i) {
		if(*f0[i].data_ptr<double>() == 0 || aperiodicity0[i] <= option->threshold) continue;
		D4CGeneralBody(x, x_length, fs, MyMaxDouble(world::kFloorF0D4C, *f0[i].data_ptr<double>()), fft_size_d4c,
					   temporal_positions[i], number_of_aperiodicities, window, window_length, &forward_real_fft,
					   &coarse_aperiodicity[1]);
		
		// Linear interpolation to convert the coarse aperiodicity into its
		// spectral representation.
		GetAperiodicity(coarse_frequency_axis, coarse_aperiodicity, number_of_aperiodicities, frequency_axis, fft_size,
						aperiodicity[i]);
	}
